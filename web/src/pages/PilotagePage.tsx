...
export default function PilotagePage() {
  ...
  return <OpsPage>
    ...
    {statsQuery.isError ? <OpsPanel><OpsState icon={WarningCircle} title="Les indicateurs ne sont pas disponibles" tone="danger" /></OpsPanel> : <>
      <div className="ops-pilotage-metrics"><OpsMetricStrip items={[{ label: 'Attendus PIA', value: stats?.attendus ?? 0, icon: ShippingContainer }, { label: 'Vus à quai', value: stats?.vusAQuai ?? 0, icon: Anchor }, { label: 'Sorties terminaux', value: stats?.sortiesTerminal ?? 0, icon: ArrowCircleUp }, { label: 'Entrées PIA', value: stats?.entreesPia ?? 0, icon: ArrowCircleDown, tone: 'success' }, { label: 'Sorties PIA', value: stats?.sortiesPia ?? 0, icon: ArrowCircleUp, tone: 'success' }]} /></div>
